...
export const getGentres = async (req, res) => {
  try {
    const { data } = await axios.get(`${base_url}/daftar-genre`);
    const $ = load(data);
    const results = [];
    let meta_data = null;
...
export const getAnimeByGentre = async (req, res) => {
  const query = req.query;
  const params = req.params;
  const slug = params.slug;
  const page = query.page || 1;
  try {
    const { data } = await axios.get(`${base_url}/genres/${slug}/page/${page}`);
    const $ = load(data);
    const results = [];
    let meta_data = null;
