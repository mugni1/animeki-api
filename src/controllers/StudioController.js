  ...
  try {
    ...
    $('.taxindex')
      ...
      .each((i, el) => {
        ...
        const total = $(el).find('.count').text().trim();
        const slug = $(el).attr('href').split('/').filter(Boolean).pop();
        results.push({ title, total, slug });
      });
    httpResponse(res, 200, 'Successfully get studio', results, meta_data);
  } catch {
    httpResponse(res, 500, 'Internal server error', null, null);
  }
};

export const getAnimeByStudio = async (req, res) => {
  const query = req.query;
  const params = req.params;
  const slug = params.slug;
  const page = query.page || 1;
  try {
    const { data } = await axios.get(`${base_url}/studio/${slug}/page/${page}`);
    const $ = load(data);
    const results = [];
    let meta_data = null;
    ...
    httpResponse(res, 200, 'Successfully get anime by studio', results, meta_data);
  } catch {
    httpResponse(res, 500, 'Internal server error', null, null);
  }
};
