...
export const getAnimeByCast = async (req, res) => {
  const query = req.query;
  const params = req.params;
  const slug = params.slug;
  const page = query.page || 1;
  try {
    const { data } = await axios.get(`${base_url}/cast/${slug}/page/${page}`);
    const $ = load(data);
    const results = [];
    let meta_data = null;
    ...
    httpResponse(res, 200, 'Successfully get animes by cast', results, meta_data);
  } catch {
